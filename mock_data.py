...
def create_mock_weather_data(path: str = "weather_data.zarr") -> xr.Dataset:
    ...
    print(
        f"Reduced Gaussian grid: {len(latitudes)} latitudes, {total_points} total points"
    )
    print(
        f"Points per latitude - Min: {min(points_per_lat)}, Max: {max(points_per_lat)}"
    )
    ...
    for i, var in enumerate(
        ["temperature_2m", "u_wind_10m", "v_wind_10m", "surface_pressure"]
    ):
        ...
        for t in range(time_points):
            ...
            data_at_t = base_pattern * (i + 1) + noise + seasonal * lat_weights
            if var == "temperature_2m":
                data_at_t += 273  # Kelvin offset
            elif var == "surface_pressure":
                data_at_t = data_at_t * 10 + 101325  # Around standard pressure
            data[t] = data_at_t

        data_vars[var] = (["time", "point"], data)
    ...
    ds = xr.Dataset(
        ...
        coords={
            "time": times,
            "point": points,
            "cell_area": ("point", cell_areas),
            # Store grid structure as attributes or coordinates
            "latitude": ("point", np.array(point_to_lat)),
            "longitude": ("point", np.array(point_to_lon_idx)),
        },
        attrs={
